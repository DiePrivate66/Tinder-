...
import { Pool } from 'pg';
import { PrismaClient as AuthPrismaClient } from '../generated/prisma';
...
async function main(): Promise<void> {
  ...
  const usersPool = new Pool({
    connectionString: getRequiredEnv('USERS_DATABASE_URL'),
  });
  ...
  try {
    const userResult = await usersPool.query<{ id: number; email: string }>(
